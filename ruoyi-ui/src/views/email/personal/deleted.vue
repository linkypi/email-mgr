<template>
  <div class="app-container">
    ...
    <div class="mail-toolbar">
      ...
      <el-button size="small" icon="el-icon-refresh-left" @click="handleRestore" :disabled="multiple">恢复</el-button>
      <el-button size="small" icon="el-icon-delete" @click="handleDeletePermanently" :disabled="multiple" type="danger">彻底删除</el-button>
        ...
        <div class="mail-list-content" v-loading="loading">
          ...
          >
            ...
            <div class="mail-content">
              <div class="mail-from">
                {{ mail.fromAddress || mail.toAddress }}
            ...
            <div class="mail-time">{{ parseTime(mail.deleteTime || mail.receiveTime || mail.sendTime, '{m}-{d} {h}:{i}') }}</div>
          </div>
        ...
        <div v-if="selectedMail" class="mail-preview-header">
          <div class="preview-info">
            <div class="preview-from">
              <strong>{{ selectedMail.fromAddress || selectedMail.toAddress }}</strong>
              <span class="preview-email">&lt;{{ selectedMail.fromAddress || selectedMail.toAddress }}&gt;</span>
            </div>
            <div class="preview-time">{{ parseTime(selectedMail.deleteTime || selectedMail.receiveTime || selectedMail.sendTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</div>
          </div>
          <div class="preview-actions">
            <el-button size="mini" icon="el-icon-refresh-left" @click="handleRestoreMail(selectedMail)">恢复</el-button>
            <el-button size="mini" icon="el-icon-delete" @click="handleDeletePermanentlyMail(selectedMail)" type="danger">彻底删除</el-button>
          </div>
        </div>
    ...
    <el-dialog title="邮件筛选" :visible.sync="showFilterDialog" width="500px">
      <el-form :model="filterForm" label-width="80px">
        ...
        <el-form-item label="邮件类型">
          <el-select v-model="filterForm.emailType" placeholder="请选择邮件类型" clearable>
            <el-option label="收件箱" value="inbox" />
            <el-option label="发件箱" value="sent" />
          </el-select>
        </el-form-item>
      </el-form>
      <div slot="footer">
...
<script>
import { listDeleted, getEmail, delEmail, restoreEmail, deletePermanently, markAsStarred, markAsImportant } from "@/api/email/personal";
...
export default {
  ...
  methods: {
        ...
        this.emailList = [
          {
            ...
            subject: '垃圾邮件示例',
            content: '这是一封垃圾邮件的内容...',
            deleteTime: '2024-01-15 16:30:00',
            status: 'deleted',
            isStarred: false,
            isImportant: false,
            emailType: 'inbox',
          ...
          {
            emailId: 7,
            toAddress: 'old@company.com',
            subject: '过期的会议通知',
            content: '关于过期会议的通知内容...',
            deleteTime: '2024-01-14 10:15:00',
            status: 'deleted',
            isStarred: false,
            isImportant: false,
            emailType: 'sent',
          ...
          },
          {
            ...
            subject: '测试邮件',
            content: '这是一封测试邮件，已被删除...',
            deleteTime: '2024-01-13 14:20:00',
            status: 'deleted',
            isStarred: true,
            isImportant: false,
            emailType: 'inbox',
    ...
    handleRestore() {
      ...
      this.$modal.confirm('是否确认恢复选中的邮件？').then(() => {
        restoreEmail(this.ids).then(() => {
          this.getList();
    ...
    /** 恢复单个邮件 */
    handleRestoreMail(mail) {
      this.$modal.confirm('是否确认恢复这封邮件？').then(() => {
        restoreEmail(mail.emailId).then(() => {
          this.getList();
          this.selectedMail = null;
          this.$modal.msgSuccess("恢复成功");
        }).catch(() => {
    ...
    handleDeletePermanently() {
      ...
      this.$modal.confirm('彻底删除后无法恢复，是否确认彻底删除选中的邮件？').then(() => {
        deletePermanently(this.ids).then(() => {
          this.getList();
          this.selectedMail = null;
    ...
    /** 彻底删除单个邮件 */
    handleDeletePermanentlyMail(mail) {
      this.$modal.confirm('彻底删除后无法恢复，是否确认彻底删除这封邮件？').then(() => {
        deletePermanently(mail.emailId).then(() => {
          this.getList();
          this.selectedMail = null;
          this.$modal.msgSuccess("彻底删除成功");
        }).catch(() => {
    ...
    handleFilter() {
      ...
      this.queryParams.emailType = this.filterForm.emailType;
