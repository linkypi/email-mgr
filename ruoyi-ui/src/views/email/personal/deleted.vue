<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="emailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="邮件ID" align="center" prop="id" width="80" />
      <el-table-column label="发件人" align="center" prop="sender" width="200" />
...
<script>
import { listDeleted, deletePermanently, restoreEmail } from "@/api/email/personal";
  ...
  methods: {
    ...
    handleRestoreFromDialog() {
      ...
      this.$modal.confirm('是否确认恢复这封邮件？').then(() => {
        restoreEmail([this.form.id]).then(() => {
          this.getList();
          this.open = false;
          this.$modal.msgSuccess("恢复成功");
        }).catch(() => {
