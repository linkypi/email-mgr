<template>
  <div class="app-container">
    ...
    <div class="mail-toolbar">
      ...
      <el-button size="small" icon="el-icon-view" @click="handleMarkAsRead" :disabled="multiple">标记已读</el-button>
        ...
        <div v-if="selectedMail" class="mail-preview-header">
          <div class="preview-info">
            <div class="preview-from">
              <strong>{{ selectedMail.fromAddress }}</strong>
          ...
          <div class="preview-actions">
            <el-button size="mini" icon="el-icon-refresh-left" @click="handleReply">回复</el-button>
...
<script>
import { listInbox, getEmail, delEmail, markAsRead, markAsStarred, markAsImportant } from "@/api/email/personal";
  ...
  methods: {
        ...
        this.emailList = [
          {
            ...
            subject: '2023年带薪休假政策更新通知',
            content: '尊敬的员工：根据最新国家政策和公司规定，我们对2023年带薪休假政策进行了以下更新...',
          ...
          {
            emailId: 2,
            fromAddress: 'tech@company.com',
            subject: '生产环境系统升级提醒',
            content: '各位同事：计划于今晚23:00至次日凌晨3:00进行生产环境系统升级，升级期间将暂停...',
          ...
          },
          {
            ...
            subject: '【周报】数字化项目进度报告 - 第25周',
            content: '项目整体进度：完成86%，本周完成需求分析会议3次，核心模块...',
    ...
    markAsRead(emailId) {
      markAsRead(emailId).then(() => {
        // 更新本地状态
        const mail = this.emailList.find(m => m.emailId === emailId);
        if (mail) {
          mail.status = 'read';
        }
        
        // 通知未读数量管理器更新
        emailUnreadManager.refreshCount('inbox');
        
        // 触发全局事件
        this.$bus.$emit('email-status-changed');
      }).catch(() => {
        // 如果API不存在，直接更新本地状态
        const mail = this.emailList.find(m => m.emailId === emailId);
        if (mail) {
    ...
    handleToggleStar(mail) {
      ...
      markAsStarred(mail.emailId).then(() => {
        ...
        // 通知未读数量管理器更新
        emailUnreadManager.refreshCount('starred');
    ...
    handleMarkAsRead() {
      ...
      this.$modal.confirm('是否确认标记选中的邮件为已读？').then(() => {
        this.emailList.forEach(mail => {
          if (mail.selected && mail.status === 'unread') {
            this.markAsRead(mail.emailId);
          }
        });
    ...
    handleReply() {
      ...
      this.$router.push({
        ...
        query: {
          replyTo: this.selectedMail.emailId,
