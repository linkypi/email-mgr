          ...
          >
            ...
            <div class="mail-content">
              ...
              <div class="mail-sender">
                <span class="sender-info">{{ mail.folderType === 'sent' ? '收件人' : '发件人' }}: {{ mail.folderType === 'sent' ? mail.recipient : mail.sender }}</span>
                <span v-if="mail.isRead === 0" class="mail-unread"></span>
              </div>
              
              <!-- 第三行：收件人信息 -->
              <div class="mail-recipient">
                <span class="recipient-info">{{ mail.folderType === 'sent' ? '发件人' : '收件人' }}: {{ mail.folderType === 'sent' ? mail.sender : mail.recipient }}</span>
              </div>
              ...
              <div class="mail-status">
                ...
                <span v-if="mail.openedTime" class="opened-time">
                  已读 {{ parseTime(mail.openedTime, '{m}-{d} {h}:{i}') }}
                </span>
            ...
            <div class="mail-time">{{ parseTime(mail.folderType === 'sent' ? mail.sentTime : mail.receiveTime, '{m}-{d} {h}:{i}') }}</div>
          </div>
        ...
        <div v-if="selectedMail" class="mail-preview-header">
          ...
          <div class="preview-actions-row">
            ...
            <div class="preview-star">
              <i class="el-icon-star-on" @click="handleUnstarMail(selectedMail)"></i>
            </div>
          ...
          <div class="preview-sender-row">
            ...
            <span class="sender-email">{{ selectedMail.fromAddress || selectedMail.sender || '我' }}</span>
          </div>
          ...
          <div class="preview-recipient-row">
            ...
            <span class="recipient-email">&lt;{{ selectedMail.type === 'sent' ? selectedMail.toAddress : selectedMail.recipient }}&gt;</span>
          </div>
        </div>
          ...
          <div v-if="selectedMail && showCompose" class="compose-form">
            ...
            <div class="compose-header">
              <div class="compose-title">
                ...
                <span class="compose-sender">{{ selectedMail.fromAddress || selectedMail.sender || '我' }}</span>
              </div>
            ...
            <div class="original-email-box">
              ...
              <div class="original-email-content">
                <div class="original-email-info">
                  <div><strong>发件人:</strong> {{ selectedMail.fromAddress || selectedMail.sender || '我' }} &lt;{{ selectedMail.fromAddress || selectedMail.sender || '我' }}&gt;</div>
                  <div><strong>发件时间:</strong> {{ parseTime(selectedMail.receiveTime || selectedMail.sendTime, '{y}年{m}月{d}日 {h}:{i}') }}</div>
                  <div><strong>收件人:</strong> {{ selectedMail.toAddress || selectedMail.recipient }} &lt;{{ selectedMail.toAddress || selectedMail.recipient }}&gt;</div>
...
<script>
import { listStarred, unmarkAsStarred, markAsImportant, markAsRead, moveToDeleted } from "@/api/email/personal";
...
export default {
  ...
  methods: {
    ...
    handleToggleStar(mail) {
      ...
      unmarkAsStarred([mail.id]).then(() => {
        mail.isStarred = 0;
        // 取消星标后从列表中移除
        this.emailList = this.emailList.filter(item => item.id !== mail.id);
        this.total = this.emailList.length;
    ...
    handleReply() {
      ...
      // 设置回复邮件信息
      this.composeForm.toAddress = this.selectedMail.fromAddress || this.selectedMail.sender;
