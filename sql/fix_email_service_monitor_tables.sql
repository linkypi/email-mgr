...
ADD COLUMN IF NOT EXISTS `monitor_enabled` tinyint(1) DEFAULT 1 COMMENT '是否启用监控：0-禁用，1-启用' AFTER `smtp_error_time`;

-- 如果表存在但缺少字段，添加缺失的字段
ALTER TABLE `email_service_monitor` 
ADD COLUMN IF NOT EXISTS `imap_error_message` text COMMENT 'IMAP错误信息' AFTER `smtp_last_check_time`;

-- 如果表存在但缺少字段，添加缺失的字段
ALTER TABLE `email_service_monitor` 
ADD COLUMN IF NOT EXISTS `smtp_error_message` text COMMENT 'SMTP错误信息' AFTER `imap_error_message`;

-- 如果表存在但缺少字段，添加缺失的字段
ALTER TABLE `email_service_monitor` 
ADD COLUMN IF NOT EXISTS `imap_error_time` datetime DEFAULT NULL COMMENT 'IMAP错误时间' AFTER `smtp_error_message`;

-- 如果表存在但缺少字段，添加缺失的字段
ALTER TABLE `email_service_monitor` 
ADD COLUMN IF NOT EXISTS `smtp_error_time` datetime DEFAULT NULL COMMENT 'SMTP错误时间' AFTER `imap_error_time`;
...
-- 插入初始数据（如果不存在）
INSERT IGNORE INTO `email_service_monitor` (`account_id`, `email_address`, `imap_status`, `smtp_status`, `monitor_enabled`, `create_time`) 
